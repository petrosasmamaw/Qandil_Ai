...
pool.on("error", (err) => {
  console.error("⚠️ PostgreSQL pool idle client warning:", err.message);
});

export const query = (text, params) => pool.query(text, params);

export const initializeDatabase = async (retryCount = 3) => {
  for (let attempt = 1; attempt <= retryCount; attempt++) {
    ...
    try {
      ...
      console.error(`⚠️ Database connection attempt ${attempt}/${retryCount} failed:`, error.message);
      if (attempt < retryCount) {
        console.log("⏳ Retrying connection in 3 seconds (waiting for Neon serverless wake up)...");
        await new Promise((r) => setTimeout(r, 3000));
      } else {
        console.error("❌ Neon PostgreSQL initialization failed after all retry attempts.");
      }
    } finally {
